...
# Saturated model without ciitime ----
mod_splitted_saturated_nociitime <- '
# regressions
wordreading_score_post ~ wordreading_score_pre + practice_cii_words_total
wordreading_score_post ~ c(0, 0, 0)*practice_length + c(0, 0, 0)*practice_frequency + c(0, 0, 0)*practice_duration
practice_cii_words_total ~ wordreading_score_pre + practice_length + practice_frequency + practice_duration
practice_length ~ wordreading_score_pre
practice_frequency ~ wordreading_score_pre
practice_duration ~ wordreading_score_pre
...
## Fit saturated path model
fit_splitted_saturated_noccitime <- sem(
  mod_splitted_saturated_nociitime,
  data = data |> 
    mutate(
      wordreading_score_prewordreading_score_pre / 1000
    )
...
fitmeasures(fit_splitted_saturated_noccitime, c('df', 'chisq', 'pvalue', 'cfi', 'rmsea'))
modificationindices(fit_splitted_saturated_noccitime, sort = T) |> head() |> dplyr::select(!contains('sepc'))
resid(fit_splitted_saturated_noccitime, type = "cor")
plot_lavaan(fit_splitted_saturated_noccitime,
            where = "browser")
parameterTable(fit_splitted_saturated_noccitime)
summary(fit_splitted_saturated_noccitime)

# resid(fit_splitted_saturated, type = "cor") # no residual correlations because saturated model

parameterestimates(fit_splitted_saturated_noccitime) |> 
  filter(
    op == '~',
  ) |> 
  arrange(
...
# regression
## Predicting reading
wordreading_score_post ~ wordreading_score_pre + wordreading_score_mid + practice_cii_words_total_semester_2
wordreading_score_mid ~ wordreading_score_pre + practice_cii_words_total_semester_1
